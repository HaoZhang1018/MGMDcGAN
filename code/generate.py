...
def generate(ir_path, vis_path, model_path, num, model_num, output_path = None):
	...
	with tf.Graph().as_default(), tf.Session() as sess:
		...
		output_image, vis_de, ir_de = G.transform(vis = SOURCE_VIS, ir = SOURCE_ir)
		...
		# output, vis_de, IR = sess.run([output_image, VIS_de, ir_us],
		#                               feed_dict = {SOURCE_VIS: vis_img, SOURCE_ir: ir_img})
		output = output[0, :, :, 0]
		end = time.time()
                imsave(output_path + '/0' + str(num) + '.png', output)
		return (end - begin)
